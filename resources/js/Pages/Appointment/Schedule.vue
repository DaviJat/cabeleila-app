...
const props = defineProps({
    ...
    clientUpcomingAppointments: Array, // Prop that receives future appointments
});

// Logged user data state helper
const currentUser = computed(() => props.loggedClient);

// Edit Mode state definitions
const editingId = ref(props.editingAppointment ? props.editingAppointment.id : null);
const isEditing = computed(() => !!editingId.value);

// Calendar component logic bounds
...
const availableTimesForSelectedDate = computed(() => {
    ...
    // If editing and the date is the same as the original appointment, include the original hour slot
...
// Form wizard state flow mapping
const step = ref(1);
...
onMounted(() => {
    ...
    if (props.editingAppointment) {
        ...
        // Pre-select current appointment date and time components
...
// Check if appointments share the same calendar week constraints (Sunday to Saturday) to trigger notifications
const isSameWeek = (dateStr1, dateStr2) => {
    // Sanitize string definitions to yield raw clean ISO structures (YYYY-MM-DD)
...
const warningModal = ref({
    isOpen: false,
    type: '', // Matches 'duplicate' or 'merge' logic tracks
    appointment: null,
    duplicateNames: '',
});
...
const handleMerge = () => {
    ...
    // Merge new services with already booked items safely, avoiding duplicates keys
    ...
    // Update form properties to target the pre-existing availability constraints
    ...
    // Swap the active pipeline context into an update/edit operation track
    editingId.value = existingApp.id;

    // Terminate warning overlay state visibility and skip forward to confirmation summary
    warningModal.value.isOpen = false;
    step.value = 3;
    window.scrollTo({ top: 0, behavior: 'smooth' });
};

// Wizard Navigation Step Checks Interceptor
const nextStep = () => {
    ...
    if (step.value === 2) {
        ...
        // Intercept schedule conflicts only if user identity is signed in and not performing a standard edit
        if (currentUser.value && !isEditing.value && props.clientUpcomingAppointments) {
            ...
            if (conflict) {
                ...
                return; // Hold thread context to display warning overlay layout
            }
        }
    }
...
// Service Catalog Model Inversion Rules
...
const handleFinalizeClick = async () => {
    ...
    try {
        await axios.post(route('clients.sendOtp'), {
